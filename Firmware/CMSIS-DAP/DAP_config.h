...
#if defined ( CDC_UART_USE_USART1_PA9_PA10 ) && defined ( SWO_PB7 )
#error "CDC_UART_USE_USART1_PA9_PA10 与 SWO_PB7 不能同时定义（共用 USART1）"
#endif
...
#define CPU_CLOCK               72000000 ///< 72MHz, Specifies the CPU Clock in Hz
...
/// This information is returned by the command \ref DAP_Info as part of <b>Capabilities</b>.
#if defined ( CDC_UART_USE_USART1_PA9_PA10 )
#define SWO_UART                0       ///< USART1 用于 CDC 时关闭 SWO UART
#else
...
/*
 * USB-CDC 虚拟串口所用 UART 与 PA9~PA13 的关系（STM32F103）：
 * - 仅 PA9(TX)+PA10(RX) 可做 USART1 标准异步 TX/RX，本工程在 CDC_UART_USE_USART1_PA9_PA10 时采用。
 * - PA11、PA12：片上 USB D-/D+，与 USB 设备功能冲突，不能当普通 UART。
 * - PA13、PA14：MCU SWD 调试口，不能当应用 CDC 串口。
 * 默认（未定义 CDC_UART_USE_USART1_PA9_PA10）：USART2 + PA2/PA3，USART1 留给 SWO（SWO_PB7 等）。
 */
#if defined ( CDC_UART_USE_USART1_PA9_PA10 )

#define USART_CDC_CLOCK(state)      RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, state)
#define USART_CDC_GPIO_CLOCK(state) RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA | RCC_APB2Periph_AFIO, state)
#define USART_CDC_REMAP()           ((void)0)
...
#define USART_CDC_BUFFER_SIZE (1024)
...
// SWDIO/TMS Pin（若 CDC 仍用 USART2/PA2，不可使用本脚位作 SWDIO）
...
#define LED_CONNECTED_RCC   RCC_APB2ENR_IOPBEN // PB0 - Connected
#define LED_CONNECTED_PORT  GPIOB
#define LED_CONNECTED_PIN   0

#define LED_RUNNING_RCC     RCC_APB2ENR_IOPBEN // PB8 - Running
