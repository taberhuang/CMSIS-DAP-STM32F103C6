 ...
 *----------------------------------------------------------------------------*/
/* 复合设备时在 CDC 前有 IAD，长度必须与 USBD_ConfigDescriptor 中宏展开一致 */
#define USBD_CDC_ACM_DESC_LEN             (USBD_MULTI_IF * USB_INTERFACE_ASSOC_DESC_SIZE + USB_INTERFACE_DESC_SIZE + 0x0013                     + \
...
/* HID 类描述符在配置中的偏移 = 配置头 + CDC 整段(含 IAD) + HID 接口描述符 */
#define USBD_HID_DESC_OFS                 (USB_CONFIGUARTION_DESC_SIZE + USBD_CDC_ACM_ENABLE * USBD_CDC_ACM_DESC_LEN + \
                                           USBD_HID_ENABLE * USB_INTERFACE_DESC_SIZE)
...
#if (USBD_MULTI_IF)
  WBVAL(0x0200), /* USB 2.00：复合设备常用，利于主机正确枚举多接口 */
#else
  WBVAL(0x0110), /* USB 1.10 */
#endif
#if (USBD_MULTI_IF)
  /* 复合设备（如 CDC+HID）勿用顶层 bDeviceClass=0x02，否则 Windows 易只当 CDC 用而忽略 HID */
  0x00,                                 /* bDeviceClass：各接口自报类 */
