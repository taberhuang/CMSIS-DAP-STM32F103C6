...
/* Hardware test: define NRESET_1MS_TOGGLE_TEST to make the main loop toggle
 * nRESET (drive low / release) every 1 ms so an oscilloscope can probe PA7.
 * This conflicts with host-side reset control via DAP; disable the macro for
 * normal debugging once the trace has been captured. */
#if defined ( NRESET_1MS_TOGGLE_TEST )
#warning "NRESET_1MS_TOGGLE_TEST: nRESET is toggling every 1 ms; do not enable this for normal debugging."
#endif

#if defined ( BLUEPILL )
#if defined ( SWD_REMAP )
#warning "BLUEPILL board: using Remapped SWD/SWC port, TDO-PB7, nRESET-PA7, TDI-PB5"
#else
#warning "BLUEPILL board: using SWDIO-PA2, SWCLK-PA4, TDO-PB7, nRESET-PA7, TDI-PB5"
#endif
#endif
...
#endif
  /* After power-on GPIO init, force both LEDs to a known off level before the
   * DAP / blink logic takes over. */
  LedConnectedOff();
  LedRunningOff();
}
...
#if defined ( NRESET_1MS_TOGGLE_TEST )
    {
      static uint8_t nreset_test_level;
      if (nreset_test_level) {
        PIN_nRESET_HIGH(); /* Release nRESET (let the pull-up take it high). */
      } else {
        PIN_nRESET_LOW();  /* Drive nRESET low. */
      }
      nreset_test_level ^= 1u;
      Delayms(1);
    }
#endif
  }
